...
| `qmax_cyclecount` | 280 | QMax 更新时循环数 | cycle | MPC 私有语义，与 cyclecount 差 110 |
| `rm` | 2521000 | 剩余容量 | µAh | = 2521 mAh |
| `charger_full` | 5797000 | FCC 满充容量 | µAh | = 5797 mAh |
| `design_capacity` | 6000000 | 设计容量 | µAh | = 6000 mAh |
| `soh` | 100 | 标准 SOH | % | QMax/Design=100.95%→截断 100% |
| `soh_new` | 97 | Xiaomi 缓存 UI SOH | % | 与 ui_soh byte0 一致 |
| `ui_soh` | 97 119 1 2 97 100 100 100 100 100 207 | UI SOH 11 字节块 | byte[] | byte0=实际 SOH, 后续=衰减档位 |
| `fcc_soh` | 0 | TI FCC_SOH | — | **MPC8011B 不支持，返回 0** |
...
**QMax=6057 > 设计容量 6000**：QMax 超过设计容量约 0.95%。上次 QMax 更新发生在第 280 次循环时，此后约 110 次循环未再更新。QMax 超标称值在 TI 算法中可出现于全新电池，但 MPC8011B 的 QMax 语义是否与 TI 完全一致尚待确认。

**FCC=5797 mAh，QMax=6057 mAh**：FCC < QMax 是正常的——FCC = f(QMax, Ra, T, I, V)，阻抗和温度/负载会降低可用容量。FCC/Design = 96.6%，与 SOH_new=97% 吻合。

**SOH=100 vs SOH_new=97**：标准 SOH 寄存器报 100%（基于 QMax/Design 比值 100.95%→截断），Xiaomi UI SOH 已降到 97%。两者算法不同——Xiaomi UI SOH 可能综合了循环次数、温度历史、阻抗趋势等因素，比单纯 QMax/Design 更激进。
...
日均 1.8 次循环属于偏重度使用。按 6000 mAh 设计容量和典型 500+ 循环寿命（80% 保持率），390 次循环已过约 60-78% 寿命周期。
...
| `rel_soh` | 0 | Relative SOH | **0 可能表示 MPC8011B 不支持/未计算** |
| `rel_soh_cyclecount` | 0 | rel_soh 循环计数 | 同上 |
| `eis_soh` | 0 | EIS SOH | **0 可能表示不支持/未计算** |
| `eis_soh_cyclecount` | 0 | EIS SOH 循环计数 | 同上 |

**rel_soh / eis_soh / fcc_soh 全为 0**：这三个 TI 兼容 SOH 指标在 MPC8011B 上均返回 0，进一步确证 MPC8011B 不使用 TI 的多 SOH 体系，只暴露标准 `soh`（基于 QMax/Design）和 Xiaomi 私有 `ui_soh`/`soh_new`。
...
| `max_temp_time` | 899857 | 距最高温时间 | 驱动 /60 = 14997 分钟 ≈ 250 小时 |
| `run_time` | 56771911 | FG 运行时间计数 | 私有 |
| `total_fw_runtime` | 0 | FG 固件累计运行时间 | 私有 |
| `time_ht` | 0 | 高温累计时间 | 未触发 |
| `time_ot` | 0 | 过温累计时间 | 未触发 |
| `over_vol_duration` | 0 | 高电压累计时间 | 未触发 |

温度管理良好：time_ht=0、time_ot=0、over_vol_duration=0，无长时间高温/过温/高压运行记录。max_life_temp=44°C 在手机充电场景中属于正常偏高范围。
...
采集时设备处于空闲/放电状态（未充电），功率学习系统不活动。充电时这组值应会变化——`start_learning` 从 0→1，`learning_power`/`action_power` 开始有值。
...
```
设计容量:    6000 mAh
QMax:        6057 mAh  (QMax/Design = 100.95% → SOH=100%)
FCC:         5797 mAh  (FCC/Design = 96.6%  → SOH_new=97%)
RM:          2521 mAh  (RSOC=44%)
TFullChgQ:   5835      (FG 内部模型满充值，比 FCC 高 38)
TRemQ:       2605      (FG 内部模型剩余值，比 RM 高 84)
循环:        390 次
QMax上次更新: 280 次时  (110 次循环未更新 QMax)
生产日期:    2024-11-18
首用日期:    2025-02-05
使用时长:    ~7.4 个月
日均循环:    ~1.8 次/天
...
|---|---|---|---|
| `soh` | 100 | QMax/Design 比值 | 是（标准寄存器） |
| `soh_new` | 97 | Xiaomi UI SOH 缓存 | 是（厂商私有） |
| `ui_soh` | 97 119 1 2 ... | MAC 0x007B 11 字节块 | 是（厂商私有） |
| `fcc_soh` | 0 | TI FCC_SOH | **否（返回 0）** |
| `rel_soh` | 0 | TI Relative SOH | **否（返回 0）** |
| `eis_soh` | 0 | TI EIS SOH | **否（返回 0）** |

### 容量数据交叉验证

| 比值 | 计算值 | 对应指标 | 一致性 |
|---|---|---|---|
| QMax / Design | 6057/6000 = 100.95% | soh=100 | 一致（截断） |
| FCC / Design | 5797/6000 = 96.6% | soh_new=97 | 基本一致（四舍五入） |
...
- `fcc_soh`/`rel_soh`/`eis_soh` 返回 0 确证 MPC8011B 不支持 TI 多 SOH 体系
